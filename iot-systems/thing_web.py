...
from webthing import (Action, Event, SingleThing, MultipleThings, Property, Thing, Value,
                      WebThingServer)
...
class VoiceControlThing(Thing):
    """A humidity sensor which updates its measurement every few seconds."""

    def __init__(self):
        Thing.__init__(
            self,
            'urn:dev:ops:voice-control-thing',
            'Voice Control Thing',
            ['VoiceCommandString'],
        ...
        self.add_property(
            Property(self,
                     ...
                     metadata={
                         'title': 'Voice Control',
                         'type': 'string',
                         'description': 'voice control command',
                         'readOnly': True,
                     }))
    ...
    def update_level(self):
        new_level = self.readAudio()
        
        logging.debug('voice command: %s', new_level)
        self.level.notify_of_external_update(new_level)
    ...
    def readAudio(self):
        ...
        with self.source as source:
            try:
                ...
                text = self.r.recognize_google(audio, language="pt-BR").lower()
            except Exception as e:
                logging.debug('voice command error: %s', e)
        return text
